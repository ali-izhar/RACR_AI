...
def sample_manifest_file(tmp_path):
    manifest_content = {
        ...
        "playbook": {
            ...
            "EDGE1": [{"task_type": "wait_for_tasks"}],
        },
    }
...
def test_experiment_manifest_get_zdeploy_params_unavailable_device(
...
class MockModelFactory(ModelFactoryInterface):
    def create_model(
        self, config_path=None, master_dict=None, flush_buffer_size=100
    ) -> ModelInterface:
...
def test_experiment_save_report(mock_experiment):
    ...
    assert (
        mock_csv_writer.return_value.writerow.call_count == 3
    )  # Header + 2 rows of data
